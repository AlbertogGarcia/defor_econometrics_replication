...
multi_group_landscape <- function(nobs, cellsize, ppoints, cpoints){
  ...
  groupcounty <- data.frame(
    county= seq(from = 1, to = length(v_county)),
    G = sample.int(3, length(v_county), replace = TRUE)+2
  )%>%
    mutate(G = ifelse(G==5, 0, G))
  
  
  # determine which pixels are treated vs. untreated
  pixloc_df <- pixloc_df %>%
    inner_join(groupcounty, by = "county")
